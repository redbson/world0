...
```
src/world0/
├── world.py              # World — unified Agent interface / 统一 Agent 接口
├── concepts/
│   └── manager.py        # Concept lifecycle / 概念生命周期管理
├── relations/
│   └── manager.py        # Relation lifecycle / 关系生命周期管理
├── dynamics/
│   ├── activation.py     # Spreading activation / 扩散激活
│   ├── decay.py          # Exponential decay / 指数衰减
│   ├── hebbian.py        # Hebbian learning / Hebbian 学习
│   └── lifecycle.py      # Maturity promotion / 成熟度晋升
├── projection/
│   └── engine.py         # MMR-based projection / 基于 MMR 的投影
├── extraction/
│   └── extractor.py      # LLM-powered extraction / LLM 驱动提取
├── llm/
│   ├── base.py           # LLMProvider interface / LLM 提供者接口
│   ├── openai.py         # OpenAI implementation / OpenAI 实现
│   └── anthropic.py      # Anthropic implementation / Anthropic 实现
├── schemas/
│   ├── concept.py        # ConceptNode data model / 概念节点数据模型
│   ├── relation.py       # RelationEdge data model / 关系边数据模型
│   └── types.py          # Observation, Projection, etc. / 输入输出类型
└── store/
    ├── base.py           # Abstract Store interface / 抽象存储接口
    └── json_store.py     # JSON file persistence / JSON 文件持久化
```
...
```bash
pip install -e ".[dev]"
pytest                        # 131 tests, ~2s / 131 个测试，约 2 秒
...
| Other tests / 其他测试 | 64 | Unit tests for decay, extraction, Hebbian, integration, lifecycle, LLM providers, persistence, projection, validation / 衰减、提取、Hebbian、集成、生命周期、LLM 提供者、持久化、投影、验证的单元测试 |
